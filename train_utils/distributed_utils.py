...
class ConfusionMatrix(object):
    ...
    def __str__(self):
        acc_global, acc, iu,f1 = self.compute()
        return (
            'global correct: {:.1f}\n'
            'average row correct: {}\n'
            'IoU: {}\n'
            'mean IoU: {:.1f}\n'
            'UNetFormer same mean IoU: {:.1f}\n'
            'F1: {}\n'
            'mean F1: {:.1f}\n'
            'UNetFormer same mean F1: {:.1f}').format(
                acc_global.item() * 100,
                ['{:.1f}'.format(i) for i in (acc * 100).tolist()],
                ['{:.1f}'.format(i) for i in (iu * 100).tolist()],
                iu.mean().item() * 100,
                iu[:-1].mean().item() * 100,
                ['{:.1f}'.format(i) for i in (f1* 100).tolist()],f1.mean().item() * 100,f1[:-1].mean().item() * 100)
